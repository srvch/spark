...
class _ContactImportSheetState extends ConsumerState<ContactImportSheet> {
  ...
  Future<void> _checkSingle() async {
    ...
    try {
      final matches = await ref.read(socialApiRepositoryProvider).matchContacts(
        [raw],
      );
      HapticFeedback.lightImpact();
      setState(() {
  ...
  Future<void> _addFriend(MatchedContact contact) async {
    ...
    try {
      await ref
          ...
          .sendFriendRequest(contact.phoneNumber);
      HapticFeedback.lightImpact();
      setState(() {
        _sent.add(contact.userId);
        _sending.remove(contact.userId);
      });
    } catch (_) {
                ...
                if (_allContacts.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Builder(
                      builder: (context) {
                        final matchedPhones =
                            _results!.map((r) => r.phoneNumber).toSet();
                        final others =
                            _allContacts.where((local) {
                              return local.phones.isNotEmpty &&
                                  !matchedPhones.contains(
                                    local.phones.first.number.replaceAll(
                                      RegExp(r'[\s\-()]+'),
                                      '',
                                    ),
                                  );
  ...
  Future<void> _inviteSms(String name, String phone) async {
    final message =
        'Hey $name, join me on Spark! It helps me plan meetups with friends easily. Get it here: https://spark.app';
    HapticFeedback.lightImpact();
